...
class UniformCV(Estimator):
    ...
    def estimate(self, query, logged_ranking, new_ranking, logged_value):
        ...
        numMatches = validDocs - numpy.count_nonzero(numpy.absolute(new_ranking-logged_ranking))
        G = 1 - validDocs + numAllowedDocs * numMatches

        '''
        lambdas = np.dot(reward * f, x-1) / num_samples / (slateDimensions - 1)
        print(lambdas)
        cv = np.mean(x - 1, axis=0) @ lambdas
        '''
