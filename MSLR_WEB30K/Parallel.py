if __name__ == "__main__":
    ...
    if args.replacement:
        print("********** args.replacement = True ************")
    else:
        print("********** args.replacement = False ************")
    ...
    if args.max_docs >= 1:
        ...
        detLogger=Policy.DeterministicPolicy(data, 'tree', False) # Line modified from original code
    ...
    targetPolicy=Policy.DeterministicPolicy(data, args.evaluation_ranker, False) # Line modified from original code
    ...
    if True:
        ...
        loggingPolicy=Policy.UniformPolicy(data, True) # Line modified from original code
    ...
    for iteration in range(args.start, args.stop):
        ...
        for j in range(args.logSize):
            ...
            numAllowedDocs=loggingPolicy.dataset.docsPerQuery[currentQuery]
            if numAllowedDocs >= args.length_ranking:
        ...
        SEs.append(saveMSEs[-1])
    
    print(SEs)
    print("std(SE):") 
    print(numpy.std(SEs))
    print("log10(sqrt(mean(SE))):") 
    print(numpy.log10(numpy.sqrt(numpy.mean(SEs))))
